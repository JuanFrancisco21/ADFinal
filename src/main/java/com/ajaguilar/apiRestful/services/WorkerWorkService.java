...
public class WorkerWorkService {
	...
	/**
	 * M�todo que nos proporciona todos los WorkerWork
	 * 
	...
	/**
	 * M�todo que proporciona un WorkerWork cuya id coincida con el valor introducido
	 * 
		...
		if (id != null) {
			try {
				...
				if (result.isPresent()) {
					...
					logger.error("Error ---> La relaci�n no existe", id + ", getWorkerWorkById");
	...
	/**
	 * M�todo que instancia un objeto WorkerWork en la base de datos
	 * 
		...
		if (workerWork != null) {
			...
			if (result != null) {
				if (result.isPresent()) {
					...
					logger.error("Error ---> La relaci�n no existe", workerWork + ", updateWorkerWork");
		...
		if (idWorker != null) {
			try {
				...
				if (result.isPresent()) {
					...
					logger.error("Error ---> La relaci�n no existe", idWorker + ", getWorkerWorkByWorker");
		...
		if (idWorker != null) {
			try {
				...
				if (result.isPresent()) {
					...
					logger.error("Error ---> La relaci�n no existe", idWorker + ", getWorkerWorkByCurrentWorker");
					throw new RecordNotFoundException("This workerwork doesn't exist, idWorker= ", idWorker);
				}
			} catch (IllegalArgumentException e) {
