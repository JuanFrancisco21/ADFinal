...
public interface WorkerWorkRepository extends JpaRepository<WorkerWork, Long> {
	 ...
	 */
	@Query(value = "SELECT * FROM workerWork where workerWork.idWorker = %?1", nativeQuery = true)
	 ...
	 * @throws IllegalArgumentException
	 */
	@Query(value = "SELECT * FROM workerWork where workerWork.idWorker = %?1 AND workerWork.current = TRUE", nativeQuery = true)
	 ...
	 * @throws IllegalArgumentException
	 */
	@Query(value = "SELECT * FROM workerWork where workerWork.idWork = %?1", nativeQuery = true)
